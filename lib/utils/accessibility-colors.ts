 ...
 * WCAG AA compliant for text and UI elements
 */

export type CrimeMetric = 'violentCrime' | 'carTheft' | 'breakIns' | 'pettyTheft';
export type SeverityLevel = 'low' | 'moderate' | 'high' | 'veryHigh';
...
/**
 * WCAG AA compliant color schemes for different severity levels
 */
export const accessibilityColorSchemes: Record<SeverityLevel, ColorScheme> = {
  low: {
    standard: '#00ff00',
    highContrast: '#00ff00',
    pattern: 'dots',
...
/**
 * Generate SVG pattern definitions for colorblind mode
 * Use this in an SVG context to enable pattern overlays
 */
...
/**
 * Determine severity level based on crime value and metric type
 */
...
/**
 * Get accessible color with optional pattern support
 */
...
/**
 * Generate CSS background patterns for non-SVG contexts (legends, UI elements)
 */
...
/**
 * Get text description for screen readers (supports i18n)
 */
export const getSeverityDescription = (
  ...
  const descriptions: Record<string, Record<SeverityLevel, string>> = {
    en: {
      low: 'low severity',
